...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $expected_profit = $quantity * $ppbottle;

    $stmt = $conn->prepare("UPDATE products SET product_name = ?, category = ?, quantity = ?, stock_price = ?, price_per_bottle = ?, expected_profit = ?, user_in_charge = ? WHERE product_id = ?");
    $stmt->bind_param("ssdddsss", $pname, $category, $quantity, $pprice, $ppbottle, $expected_profit, $user_in_charge, $productId);
    
    if ($stmt->execute()) {
        // Setting a success flag in the session
        $_SESSION['product_updated'] = true;
        header("Location: ayour_form_page.php"); // Redirect to the main stock page
        exit;
    } else {
